 ...
 */
const loadAudio = async (audioPath, mimeType) => {
  // Handle YouTube URLs
...
export const nativeTools = [
  ...
  },
  {
    ...
    parameters: {
      ...
      required: ["audio_path", "mime_type", "question"],
      additionalProperties: false
    },
    strict: false
  },
...
export const nativeHandlers = {
  ...
  async transcribe_audio({ 
    ...
    try {
      const audio = await loadAudio(audio_path, mime_type);
      ...
      if (output_name) {
        const outputDir = "05_01_task/workspace/output";
        ...
        return {
          success: true,
          audio_path,
          output_path: `05_01_task/workspace/output/${output_name}_${timestamp}.json`,
   ...
   */
  async analyze_audio({ audio_path, mime_type, analysis_type = "general", custom_prompt, output_name }) {
    log.tool("analyze_audio", { audio_path, mime_type, analysis_type });
   ...
   */
  async query_audio({ audio_path, mime_type, question }) {
    log.tool("query_audio", { audio_path, mime_type, question: question.substring(0, 50) + "..." });
