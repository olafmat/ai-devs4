...
import { nativeTools as nativeAudioTools, isNativeTool as isNativeAudioTool, executeNativeTool as executeNativeAudioTool} from "./native/audio/tools.js";
import { nativeTools as nativeImageTools, isNativeTool as isNativeImageTool, executeNativeTool as executeNativeImageTool} from "./native/image/tools.js";
...
const runTool = async (mcpClient, toolCall) => {
  ...
  try {
    const result =
      aiDevsHandlers[toolCall.name] !== undefined
      ? await aiDevsHandlers[toolCall.name](args)
      : isNativeAudioTool(toolCall.name)
      ? await executeNativeAudioTool(toolCall.name, args)
      : isNativeImageTool(toolCall.name)
      ? await executeNativeImageTool(toolCall.name, args)
...
export const run = async (query, { mcpClient, mcpTools, conversationHistory = [] }) => {
  const tools = [...aiDevsTools, ...mcpToolsToOpenAI(mcpTools), ...nativeAudioTools, ...nativeImageTools];
