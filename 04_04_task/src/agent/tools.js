...
const TOOLS = [
  ...
  // ── Workspace utilities ──────────────────────────────────────
  {
    ...
    name: "download_data",
    description:
      "Download the zip data and extract it into workspace.",
    parameters: {
      type: "object",
      properties: {
      },
  ...
  },
  {
    ...
    description: "Creates a new directory under an existing parent directory.",
    parameters: {
      type: "object",
      properties: {
        path: {
          type: "string",
          description: `path, for example "/path"`,
        },
      },
      required: ["path"],
    },
  ...
  },
  {
    type: "function",
    name: "create_file",
    description: "Creates a new file or overwrites an existing one at the same path",
    parameters: {
      type: "object",
      properties: {
        path: {
  ...
  },
  {
    type: "function",
    name: "delete_file",
    description: "Deletes file in the virtual filesystem",
    parameters: {
      type: "object",
      properties: {
        path: {
  ...
  {
    type: "function",
    name: "reset_filesystem",
    description: "Clears the whole virtual filesystem and restores only the root directory.",
    parameters: {
      type: "object",
      properties: {
      },
  ...
  },
  {
    type: "function",
    name: "done",
    description: "Runs final validation of the filesystem against all task rules.",
    parameters: {
      type: "object",
      properties: {
      },
...
export const createTools = (driver) => {
  const handlers = {
    ...
    download_data: async ({ filename }) => {
      if (!AIDEVS_KEY) return { error: "AIDEVS_KEY is not set" };

      const response = await fetch(`${HUB_URL}/dane/natan_notes.zip`, { method: "GET" });
      if (!response.ok || !response.body) {
        throw new Error(`Failed to download natan_notes.zip (${response.status})`);
      }
      ...
      for await (const entry of zipStream.pipe(unzipper.Parse({ forceStream: true }))) {
        ...
        console.log(`Created ${path} len ${text.length}`)
