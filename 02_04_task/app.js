...
const query = `
I am working on a task from AIDevs course, which provides a simulated API with a set of messages.
We need to search the messages and extract three pieces of information:
when (format: YYYY-MM-DD) a power plant is going to be attacked.
the password for the employee system, which is inside one of the messages
the confirmation code from the ticket (format: SEC- + 32 characters = 36 characters total)
New messages may arrive while you're working. You need to keep that in mind.
Wiktor sent a message from the proton.me domain.
The API supports the operators from:, to:, subject:, OR, and AND.
`;
