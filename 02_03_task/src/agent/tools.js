...
import { AIDEVS_KEY, HUB_URL } from "../../../config.js";
...
const TOOLS = [
  ...
  // ── Workspace utilities ──────────────────────────────────────
  {
    ...
    name: "download_logs",
    description:
      "Download the log file and save it into workspace." +
      "Use when you need to inspect the hub's failure logs locally.",
    parameters: {
      type: "object",
      properties: {
        filename: {
  ...
  },
  {
    type: "function",
    name: "submit_logs",
    description: "Send the condensed version of logs",
    parameters: {
      type: "object",
      properties: {
        logs: {
          type: "string",
          description: `The condensed version of logs. 
            Each line of the text should be one event and contain date in format YYYY-MM-DD, time in format HH:MM or H:MM.
            It shouldn't exceed 1500 tokens, you can shorten of paraphrase the original logs, but keep timestamp and severtity level.`,
        },
      },
      required: ["logs"],
    },
  ...
  },
  {
    ...
    name: "count_tokens",
    description:
      "Count the approximate number of tokens in a string using an OpenAI-compatible tokenizer (tiktoken). " +
      "Use this to keep prompts under model limits.",
    parameters: {
      type: "object",
      properties: {
        text: {
        ...
        model: {
          type: "string",
          description:
            "Model name to select tokenizer mapping (default: gpt-4o). If unknown, falls back to o200k_base.",
        },
      },
      required: ["text"],
    },
...
export const createTools = (driver) => {
  const handlers = {
    ...
    download_logs: async ({ filename }) => {
      if (!AIDEVS_KEY) return { error: "AIDEVS_KEY is not set" };

      await mkdir(WORKSPACE_DIR, { recursive: true });

      const url = `${HUB_URL}/data/${AIDEVS_KEY}/failure.log`;
      const response = await fetch(url);
      if (!response.ok) {
        return { error: `Failed to download the log (HTTP ${response.status})` };
      }

      const data = await response.text();
      await writeFile(join(WORKSPACE_DIR, filename), data, "utf-8");

      return { success: true, savedAs: `${WORKSPACE_DIR}/${filename}`, bytes: Buffer.byteLength(data, "utf-8") };
    },

    submit_logs: async ({ logs }) => {
      if (!AIDEVS_KEY) return { error: "AIDEVS_KEY is not set" };
      if (typeof logs !== "string" || !logs.trim()) return { error: "logs must be a non-empty string" };
      const tokens = handlers.count_tokens(logs)
      if (tokens > 1500) {
        return {
          error: "The number of tokens in logs must not exceed 1500",
      ...
      if (!response.ok) {
        return {
          error: `Verify request failed (HTTP ${response.status})`,
          response: payload,
        };
      }
    ...
    count_tokens: async ({ text, model = "gpt-4o" }) => {
      ...
      try {
        return { numberOfTokens: enc.encode(input).length };
      } finally {
        enc.free();
      }
    },
